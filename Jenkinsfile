pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'riman/techsolutions-app'
        K8S_CLUSTER_NAME = 'riman-cluster'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/RimanSingh/microservices-ingress.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|riman/techsolutions-app:latest|riman/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
